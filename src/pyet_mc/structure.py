...
class Structure:
    ...
    def __init__(self, cif_file: str):
        ...
        Raises:
        FileNotFoundError: If the CIF file path does not exist.
        ValueError: If the CIF file cannot be parsed.
        """
        if not os.path.isfile(cif_file):
            raise FileNotFoundError(
                f"CIF file not found: {cif_file!r}. "
                f"Please provide a valid path to a .cif file."
            )
        self.filename = os.path.basename(cif_file)
        try:
            self.cif = CifParser(cif_file)
            self.struct = self.cif.parse_structures(primitive=False)[0]
        except Exception as e:
            raise ValueError(f"Failed to parse CIF file '{cif_file}': {e}") from e
    ...
    def centre_ion(self, ion: str) -> None:
        ...
        Raises:
        ValueError: If the specified ion is not found in the structure.
        """
        ...
        for i in range(len(structure_sites)):
            ...
            if temp.species_string == ion:
                self.ion_index = i
                self.site = self.struct.sites[self.ion_index]
                ...
                break
        if not found:
            available = sorted(set(s.species_string for s in structure_sites))
            raise ValueError(
                f"Ion '{ion}' not found in the structure. "
    ...
    def get_R0(self) -> None:
        ...
        print(
            f"with a nearest neighbour {self.centre_ion_species} at {self.r0} angstroms"
        )
    ...
    def nearest_neighbours_info(self, radius: float) -> None:
        ...
        for n in self.nn:
            s += "Species = %s, r = %f Angstrom\n" % (
                n[0].species.chemical_system,
                self.site.distance(n[0]),
            )
        s += "\n"
        print(s)
    ...
    def structure_plot(
        self, radius: float, filter: Optional[List[str]] = None
        ...
        fig.structure_3d(
            ...
            np.array([self.origin[2]]),
            name=f"central {self.centre_ion_species} ion",
            marker={"color": "black"},
        )
        if filter is not None:
            ...
            if any(item not in DataFrameDict for item in filter):
                raise KeyError(
                    "Some elements in the filter list are not keys of the DataFrame."
                )
        ...
        if filter == None:
            ...
            for i in range(len(filter)):
                ...
                fig.structure_3d(
                    temp.x, temp.y, temp.z, name=filter[i], marker={"color": colour}
                )
...
class Interaction:
    ...
    distance_sim(self, radius): Computes spherical coordinates of same-species ions within a given radius and stores them in self.filtered_coords.
    doper(self, concentration, dopant, return_coords): Randomly assigns dopant species to filtered_coords and returns radial distances (default) or full DataFrame with r, theta, phi, species (return_coords=True). Must call distance_sim() first.
    sim_single_cross(self, radius, concentration, iterations, interaction_type): Simulates a single cross-relaxation interaction within a given radius (calls distance_sim and doper internally).
    doped_structure_plot(self, radius, concentration, dopant, filter): Generates a 3D plot of the structure including dopant ions.
    """
    ...
    def __init__(self, structure: Structure):
        ...
        try:
            ...
            print(
                f"Structure loaded, the central ion is {self.structure.centre_ion_species}"
            )
        except Exception as e:
            print(
                f"Either structure or central ion has not been correctly specified: {e}"
            )

    def distance_sim(self, radius: float) -> None:
        """
        Computes the spherical coordinates of same-species ions within a given radius
        of the central ion and stores them in self.filtered_coords. This must be called
        before calling doper().

        Parameters:
        radius (float/int): The radius within which to find same-species ions.

        Returns:
        None. Sets self.filtered_coords (pandas.DataFrame) as a side effect.
        ...
        self.filtered_coords = coords.loc[
            coords["species"].isin([self.structure.centre_ion_species])
        ].reset_index(drop=True)

    def doper(
        self,
        concentration: float,
        dopant: Optional[str] = "acceptor",
        return_coords: bool = False,
    ) -> Union[np.ndarray, pd.DataFrame]:
        """
        Randomly assigns dopant species to the filtered coordinates based on the given
        concentration and returns the radial distances of the resulting dopant ions.
        distance_sim() must be called before this method.

        Parameters:
        concentration (float): The concentration of the dopant in % (e.g. 15 for 15%).
        dopant (str, optional): The species label to assign to doped sites. Defaults to 'acceptor'.
        return_coords (bool, optional): If True, returns a DataFrame with r, theta, phi,
            and species columns showing the full doped configuration (both doped and
            undoped sites). If False (default), returns only the radial distances of
            the dopant ions as a numpy array.

        Returns:
        numpy.ndarray: An array of radial distances (in Angstroms) to the dopant ions (default).
        pandas.DataFrame: The full doped configuration with r, theta, phi, species columns
            (when return_coords=True).

        Example:
            crystal_interaction.distance_sim(radius=10)

            # Get just the radial distances (default)
            distances = crystal_interaction.doper(concentration=15, dopant='Sm3+')

            # Get the full doped configuration as a DataFrame
            doped_config = crystal_interaction.doper(concentration=15, dopant='Sm3+', return_coords=True)
        """
        if not hasattr(self, "filtered_coords"):
            raise AttributeError(
                "filtered_coords not found. You must call distance_sim() "
                "before calling doper(). Example:\n"
                "  interaction.distance_sim(radius=10)\n"
                "  distances = interaction.doper(concentration=15)"
            )
        ...
        if return_coords:
            result = self.filtered_coords.copy()
        else:
            result = self.filtered_coords.loc[
                self.filtered_coords["species"] == dopant, "r"
            ].values
    ...
    def sim_single_cross(
        ...
        """
        process = "singlecross"
        cache_data = pyet_utils.cache_reader(
            sourcefile=self.structure.filename,
            process=process,
            radius=radius,
    ...
    def doped_structure_plot(
        ...
        filtered_coords = coords.loc[
            coords["species"].isin([self.structure.centre_ion_species])
        ].copy()

        coords = coords.drop(
            coords.index[coords["species"] == self.structure.centre_ion_species]
        )
        ...
        fig = Plot()
        fig.structure_3d(
            np.array([self.structure.origin[0]]),
            np.array([self.structure.origin[1]]),
            np.array([self.structure.origin[2]]),
            name=f"central {dopant} ion",
            marker={"color": "black"},
        )
        if filter == None:
            for i in range(len(UniqueNames)):
            ...
            try:
                ...
                print(f'Failed to plot. Filter must be in type "list of strings": {e}')
        return fig
...
if __name__ == "__main__":
    ...
    cif_dir = os.path.abspath(
        os.path.join(os.path.dirname(__file__), "..", "cif_files")
    )
    ...
    interaction_components = crystal_interaction.sim_single_cross(
        radius=10, concentration=2.5, iterations=50000, interaction_type="DQ"
    )
