    ...
    def __init__(
        ...
        if parser:
            match parser:
                ...
                case _:
                    print(
                        "In correct parsing function these are the currently available parsing functions\n 'parse_10'\n 'parse_100' \n"
                    )
...
def cache_reader(sourcefile: str, **params) -> np.ndarray:
    ...
    vmat = [
        params["process"],
        str(params["radius"]),
        str(params["concentration"]),
        str(params["interaction_type"]),
        str(params["iterations"]),
        "intrinsic",
        str(params["intrinsic"]),
    ]
    ...
    try:
        ...
        if data is None:
            raise FileNotFoundError(
                "No cached file found matching the given parameters. "
                "Check your inputs or consider running a simulation with these parameters."
            )
...
def cache_clear(index: Optional[int] = None) -> None:
    ...
    if index is None:
        ...
        print(
            f"File to delete: {os.path.basename(file)} Source file: {sourcefile}, Date created: {date}"
        )
...
def cache_list() -> None:
    ...
    for index, file in enumerate(files):
        ...
        with open(file, "r") as f:
            ...
            date = data.get("date", "N/A")
        print(
            f"[{index}] {os.path.basename(file)} ({file_size} bytes), Source file: {sourcefile}, Date created: {date}"
        )
...
def random_spectra(wavelength: np.ndarray, wavenumbers: bool = False) -> np.ndarray:
    ...
    LINE1 = (
        stats.norm.pdf(
            wavelength,
            rd.uniform(np.min(wavelength) + 10, np.max(wavelength) - 10),
            sigma1,
        )
        * 1000
    )
